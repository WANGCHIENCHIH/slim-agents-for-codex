...
- A terminal result, including its findings, validation evidence, and shared-worktree effects, must be reconciled before redispatch. Do not dispatch the same approved objective again before that reconciliation. After reconciliation, use followup_task for retained context or a clearly scoped replacement only when approved work remains.
- When a terminal result is empty or unusable, inspect the actual artifacts, shared-worktree changes, and validation evidence first. If that evidence satisfies the assigned acceptance criteria, reconcile the lane without repeating completed work. Otherwise keep it unresolved and use followup_task with the retained specialist to request only the missing report, evidence, or work; replace it only under the existing lifecycle rules. An empty reply alone justifies neither completion nor redispatch.
