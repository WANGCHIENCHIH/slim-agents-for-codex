...
describe("preset generation", () => {
  ...
  it("uses the current model and effort mappings for each generation", () => {
    expect(resolvePreset("openai-5.5").models).toEqual({
      ...
      oracle: { model: "gpt-5.5", effort: "high" },
      librarian: { model: "gpt-5.4-mini", effort: "low" },
      explorer: { model: "gpt-5.4-mini", effort: "low" },
      designer: { model: "gpt-5.4-mini", effort: "medium" },
      fixer: { model: "gpt-5.5", effort: "low" },
      council: { model: "gpt-5.5", effort: "high" },
    });
    expect(resolvePreset("openai-5.6").models).toEqual({
  ...
  it("ports the reviewed upstream specialist prompts into generated developer instructions", () => {
    ...
    expect(instructions.fixer).toMatch(/<summary>.*<changes>.*<verification>.*skipped with reason/is);
  ...
  it("encodes bounded recursive orchestration for the five Slim specialists", () => {
    const prompt = generatePreset("openai-5.6").roles.orchestrator.instructions;
    for (const name of ["explorer", "librarian", "oracle", "designer", "fixer"]) expect(prompt).toMatch(new RegExp(`\\b${name}\\b`, "i"));
  ...
  it("lets council assemble installed experts without recursive council fan-out", () => {
    ...
    expect(prompt).toMatch(/untrusted data.*rather than instructions/is);
  ...
  it("generates the exact behavioral MCP denylist for every role and preset", () => {
    const expected: Record<string, string[]> = {
      orchestrator: ["context7", "exa", "grep"],
    ...
    for (const id of Object.keys(presets)) {
      ...
      for (const [name, toml] of Object.entries(generated.agents)) {
        ...
        const deniedMcps = expected[name];
  ...
  it("identifies generated manifests by Package Version and Preset ID", async () => {
    const packageDocument = JSON.parse(await readFile(join(process.cwd(), "package.json"), "utf8")) as { version: string };
    for (const id of Object.keys(presets)) {
      expect(JSON.parse(generatePreset(id).manifest)).toMatchObject({ packageVersion: packageDocument.version, presetId: id });
    }
  });
...
describe("CLI", () => {
  ...
  it("rejects every retired suffix ID before conversion writes", async () => {
    const outputRoot = await mkdtemp(join(tmpdir(), "slim-retired-convert-"));
    for (const id of ["openai-5.5.1", "openai-5.6.1", "openai-5.6.2", "openai-5.6.3", "openai-5.6.4"]) {
      const replacement = id.startsWith("openai-5.5") ? "openai-5.5" : "openai-5.6";
      await expect(runCli(["convert", "--preset", id, "--output", outputRoot], { log: () => undefined, confirm: async () => false })).rejects.toThrow(new RegExp(`Retired preset: ${id}.*Use ${replacement}.*Package Version.*Git tag`, "i"));
  ...
  it("convert --check rejects a missing selected manifest without mutating files", async () => {
    ...
    await unlink(join(outputRoot, "openai-5.6", "manifest.json"));
    const drifted = 'name = "explorer"\n# drift stays drifted when check fails\n';
    await writeFile(join(outputRoot, "openai-5.6", "agents", "explorer.toml"), drifted, "utf8");

    await expect(runCli(["convert", "--preset", "openai-5.6", "--output", outputRoot, "--check"], { log: () => undefined, confirm: async () => false })).rejects.toThrow(/manifest|drift|snapshot/i);
    await expect(readFile(join(outputRoot, "openai-5.6", "manifest.json"), "utf8")).rejects.toThrow();
    expect(await readFile(join(outputRoot, "openai-5.6", "agents", "explorer.toml"), "utf8")).toBe(drifted);
  });
  ...
  it("convert --check rejects extra stale managed agent files without deleting them", async () => {
    ...
    await copyPresetSnapshot("openai-5.6", outputRoot);
    const staleObserver = join(outputRoot, "openai-5.6", "agents", "observer.toml");
  ...
  it("convert generation removes stale managed roles but preserves unrelated TOMLs", async () => {
    ...
    await copyPresetSnapshot("openai-5.6", outputRoot);
    const agentsPath = join(outputRoot, "openai-5.6", "agents");
    ...
    expect(await runCli(["convert", "--preset", "openai-5.6", "--output", outputRoot], { log: () => undefined, confirm: async () => false })).toBe(0);
  ...
  it("validate --path rejects semantic role drift beyond truthy fields", async () => {
    ...
    await expect(runCli(["validate", "--preset", "openai-5.6", "--path", agentsPath], { log: () => undefined, confirm: async () => false })).rejects.toThrow(/explorer|invalid|drift|sandbox/i);
  });

  it("validate --codex-home rejects drifted installed role semantics", async () => {
    const { home } = await createCodexHomeFromPreset("openai-5.6");
    const original = await readFile(join(home, "agents", "oracle.toml"), "utf8");
    const drifted = original.replace('description = "Strategic technical advisor. Use for architecture decisions, complex debugging, code review, simplification, and engineering guidance."', 'description = "Drifted oracle description"');
    expect(drifted).not.toBe(original);
    await writeFile(
    ...
    await expect(runCli(["validate", "--preset", "openai-5.6", "--codex-home", home, "--skills-home", packagedSkillsHome], { log: () => undefined, confirm: async () => false })).rejects.toThrow(/oracle|invalid|drift|description/i);
  });
});
